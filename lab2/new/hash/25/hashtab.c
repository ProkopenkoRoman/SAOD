...
void hashtab_double_add(listnode **hashtab, char *key, int value) {
	listnode *node;
	int index = add_hash(key, HASHTAB_SIZE);
	// Вставка в начало списка
	node = malloc(sizeof(*node));
	if (node != NULL) {
		node->key = key;
...
listnode *hashtab_add_lookup(listnode **hashtab, char *key) {
	int index;
	listnode *node;

	index = add_hash(key, HASHTAB_SIZE);
	for (node = hashtab[index];	node != NULL; node = node->next) {
		if (strcmp(node->key, key) == 0)
			return node;
	}
